...
function redirect_user_info() {
global $s,$m;
header ("Location: http://www.violinpractice.com/Violin-Articles-21/members-only-area.html");
exit;
}

##################################################################################

function get_categories_list($n) {
global $s;
$s[this_cat_n] = $this_category = $n;
//$this_url = preg_replace("/page-[0-9]+\//",'',$s[this_url]);
if ($s[this_cat_n]) $where = "(parent = '$s[this_cat_n]' or n = '$s[this_cat_n]')"; 
else $where = "level = '1'";
$category_vars = get_category_variables($n); if ($category_vars[use_for]!='v') return false;
$q = dq("select * from $s[pr]cats where $where and use_for = '$category_vars[use_for]' and visible = 1 order by level,cat_group,name",1);
while ($x=mysql_fetch_assoc($q)) { $a[$x[n]] = $x; $numbers[] = $x[n]; if ($x[bigboss]) $this_bigboss = $x[bigboss]; }
$this_category = $a[$n];
...
{ unset($a);
  $last_level = 1;
  $q = dq("select * from $s[pr]cats where parent = '$this_category[parent]' order by cat_group,name",1);
  ...
  { //if ($s[show_left_items]) $left_items = ' ('.$items[$category[n]][items].')';
    ...
    if ($category[alias_of]) $category[name] = $s[alias_pref].$category[name].$s[alias_after];
    if ($category[n]==$s[this_cat_n]) $categories .= '<p>'.$pomlcek.'<a href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'"><b>'.$category[name].$left_items.'</b></a></p>';
    else $categories .= '<p>'.$pomlcek.'<a href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'">'.$category[name].$left_items.'</a></p>';
  }
}
else
{ //if ($s[show_left_items]) $items = get_item_numbers_cats($s[this_area],$numbers,$s[this_offer_wanted]);
  ...
  { //if ($s[show_left_items]) $left_items = ' ('.$items[$category[n]][items].')';
    ...
    if ($s[this_cat_n])
    { $pomlcek = $category[level] - 1; $pomlcek = str_repeat ('-',$pomlcek);
      //if ($category[n]==$s[this_cat_n]) $categories .= '<p>'.$pomlcek.'5<a href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'"><b>'.$category[name].$left_items.'</b></a></p>';
      //else $categories .= '<p>'.$pomlcek.'<a href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'">'.$category[name].$left_items.'</a></p>';
    }
    else // first level
    $categories .= '<p>'.$pomlcek.'4<a href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'">'.$category[name].$left_items.'</a></p>';
  }
}
//echo $categories;

if (($category_vars[parent]) and (!$last_level))
{ $q = dq("select * from $s[pr]cats where use_for = '$category_vars[use_for]' and parent = '$category_vars[parent]' and visible = 1 order by level,cat_group,name",1);
  while ($category=mysql_fetch_assoc($q))
  { $pomlcek = $category[level] - 1; $pomlcek = str_repeat ('-',$pomlcek);
    if ($category[n]==$category_vars[n]) $categories1 .= '<p>'.$pomlcek.'<a href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'"><b>'.$category[name].$left_items.'</b></a></p>';
	else $categories1 .= '<p>'.$pomlcek.'<a href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'">'.$category[name].$left_items.'</a></p>';
  }
}
return $categories.$categories1;
//if ($categories1) $categories1 = str_replace('--categories--',$categories,$categories1);
//else $categories1 = $categories;

//echo $categories1;exit;
return $categories1;
}

##################################################################################

function get_category_tree($category_array) {
global $s;
...
$q = dq("select * from $s[pr]cats where $query order by level,cat_group",1);
while ($x=mysql_fetch_assoc($q)) { $a[] = $x; $numbers[] = $x[n]; }
...
{ //if ($s[show_left_items]) $left_items = ' ('.$items[$category[n]][items].')';
  ...
  $folder_icon = folder_icon(0,$category[image2]);
  if ($category[alias_of]) $category[name] = $s[alias_pref].$category[name].$s[alias_after];
  if ($category[n]==$s[this_category]) $categories .= '<p>'.$pomlcek.'<a class="link10" href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'"><b>'.$category[name].$left_items.'</b></a></p>';
  else $categories .= '<p>'.$pomlcek.'<a class="link10" href="'.category_url($category[use_for],$category[n],$category[alias_of],$category[name],1,$category[rewrite_url]).'">'.$category[name].$left_items.'</a></p>';
}
return $categories;
}
...
foreach ($files[$a[n]] as $k => $v) $a[files] .= '<a href="'.$v[url].'" style="font-size:22px;"><img border="0" src="'.$s[site_url].'/images/pdf_large.png" style="margin-bottom:-15px;">'.$v[description].'</a><br>';
...
/*
  if ((substr($a[picture],-3))=='pdf') $a[pdf] = '<tr><td align="left" valign="top" width="50%"><img border="0" src="'.$s[site_url].'/images/pdf.gif"><a href="'.$a[picture].'" target="_blank">PDF</a></td></tr>';
  else $a[pdf] = '';
*/
...
$x = explode(' ',trim($a[c])); $x = explode('_',trim($x[0]));foreach ($x as $k => $v) if ($v) $x1[] = $v;
$array_count = count($x1);
$a[category_tree] = get_categories_list($x1[($array_count-1)]);
$b = get_category_variables($x[0]); $x = preparse_ads_in_category($b); $a = array_merge((array)$a,(array)$x);
...
if ($a[description]) $a[meta_description] = $a[description]; else $a[meta_description] = $category_vars[m_desc];
...
$vl[site_news] = index_site_news(0);
...
if ($vl[category_tree]) $vl[category_tree_div] = 'block'; else $vl[category_tree_div] = 'none';

if ($s[LUG_u_n])
{ $vl[hide_for_user_begin] = '<!--'; $vl[hide_for_user_end] = '-->';
  $vl[LUG_u_n] = $s[LUG_u_n];
  $vl[LUG_u_username] = $s[LUG_u_username];
  $vl[LUG_u_password] = $s[LUG_u_password];
  $vl[hide_div_user] = ' style="display:none;"';
  $vl[show_div_user] = 'block';
  $vl[show_div_no_user] = 'none';
  $check_field = check_field_create("$s[LUG_u_username]$s[LUG_u_password]$s[LUG_u_n]");
  $vl[gc_login_data] = base64_encode("$s[LUG_u_n]-$s[LUG_u_username]");
}
else
{ //$vl[user_login_form] = user_login_form();
  ...
  $vl[show_div_user] = 'none';
  $vl[show_div_no_user] = 'block';
}

if (!$vl[share_it]) $vl[share_it] = parse_part('share_it.txt',$a);
if (!$vl[bookmark_title]) $vl[bookmark_title] = $s[site_name];
$vl[menu] = implode('',file("$s[phppath]/data/menu.txt")); if (!$s[LUG_u_n]) $vl[menu] = str_replace('#%hide_div_no_user%#','style="display:none;"',$vl[menu]);
$vl[other_url_gc1] = $s[other_gc1_url];
$vl[other_url_gc2] = $s[other_gc2_url];
...
if (!trim($vl[original_phrase])) $vl[original_phrase] = 'SEARCH';
...
$line = str_replace("$s[site_url]/uploads/pdf/","$s[site_url]/redir.php?file=/uploads/pdf/",$line);
...
echo stripslashes(str_replace('</head>','<LINK href="'.$s[site_url].'/styles/'.$s[LUG_style].'/styles.css" rel="StyleSheet"></head>',$line));
exit;
}
...
{ foreach ($all_user_items_list as $k=>$v)
  { $filter_now = $only_with_value;
    ...
    { if (($s[search_highlight]) AND ($s[highlight])) $data[value] = highlight_words('',$data[value]);
      ...
      if ( ($use_for=='c_v') AND (substr($data[value],0,7)=='http://') ) $c[$v[item_n]][$v1] = '<a href="'.$data[value].'" style="text-decoration:underline;"><b>'.$data[name].'</b></a><br>';
    }
  }
}
foreach ($all_user_items_list as $k=>$v)
...
global $s,$m;
if ($what=='v') unset($s[category_use_ajax]);
if ($total<=1)
{ if (!$total) $total = 0;
  $a[total] = $total;
  $a[hide_pages_list_begin] = '<!--'; $a[hide_pages_list_end] = '-->';
...
global $s;
//$s['v_sortby'] = 'youtube_length'; if ($what=='v') $allowed_sortby[] = 'youtube_length';
...
if (!$who) return false;
if (!$s[LUG_u_n]) redirect_user_info();//problem($reg_only);
if ($who==2)
...
{ $user_vars = get_user_variables($s[LUG_u_n]);
  if (($user_vars[n]) AND ($user_vars[username]==$s[LUG_u_username]) AND ($user_vars[password]==$s[LUG_u_password]) AND ($user_vars[paid_until]>$s[cas]))
  { $q = dq("select * from $s[pr]users_ip where user = '$user_vars[n]' and ipnum != '$s[ipnum]' and time > ($s[cas]-900)",1);
    $x = mysql_fetch_assoc($q);
    if ($x[user]) problem("It's not allowed to access the same user account from multiple IP addresses");
    dq("insert into $s[pr]users_ip values ('$user_vars[n]','$s[ipnum]','$s[cas]')",1);
    return true;
  }
}

if (strstr($what,'c_'))
{ if ((($what=='c_l') AND ($s[users_only_links_c])) OR (($what=='c_a') AND ($s[users_only_articles_c])) OR (($what=='c_b') AND ($s[users_only_blogs_c])) OR (($what=='c_v') AND ($s[users_only_videos_c])) OR (($what=='c_n') AND ($s[users_only_news_c]))) redirect_user_info();//problem($m[reg_users_only]);
  if (!$category_vars[n]) $category_vars = get_category_variables($c);
  if ($category_vars[users_only_cat]) redirect_user_info();//problem($m[reg_users_only]);
  return true;
}
if ($s['users_only_'.$s[items_types_words][$what]]) redirect_user_info();//problem($m[reg_users_only]);
$c = explode(' ',str_replace('_','',$c));
foreach ($c as $k=>$v) { $category_vars = get_category_variables($v); if ($category_vars[users_only_items]) redirect_user_info();/*problem($m[reg_users_only]);*/ }
}
