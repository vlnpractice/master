...
switch ($_GET[action]) {
case 'user_subscription'	: user_subscription($_GET);
case 'logged_in'			: logged_in($_GET);
case 'login'				: user_login_page($_GET);
...
case 'user_join'			: user_join($_GET);
}
switch ($_POST[action]) {
case 'user_order'			: user_order($_POST);
...
function user_subscription($in) {
global $s,$m;
$in[n] = 0;
$user_vars = get_user_variables($s[LUG_u_n]);
if ($user_vars[paid_until]>$s[cas])
{ $a[hide_order_begin] = '<!--'; $a[hide_order_end] = '-->'; 
  $a[paid_until] = datum($user_vars[paid_until]);
}
else
{ $a[hide_subscription_begin] = '<!--'; $a[hide_subscription_end] = '-->'; 
  $q = dq("select * from $s[pr]users_prices order by months",1);
  while ($x=mysql_fetch_assoc($q)) $a[order_options] .= '<input type="radio" name="months" value="'.$x[months].'">'.$x[months].' months for '.$s[currency].$x[price].'<br>';
}
if ($s[info]) $a[info] = $s[info];
page_from_template('user_subscription.html',$a);
}

#########################################################################

function user_order($in) {
global $s,$m;
$months = round($in[months]);
$q = dq("select * from $s[pr]users_prices where months = '$months'",1);
$a=mysql_fetch_assoc($q);
if (!$a[months]) user_subscription($in);

if (($a[coupon]) AND ($a[coupon]==trim($in[discount]))) $discount = 100;
elseif (trim($in[discount]))
{ for ($x=1;$x<=10;$x++) if (trim($in[discount])==$s["discount_code$x"]) { $discount = $s["discount$x"]; break; }
  if (!$discount) { $s[info] = 'Discount code is not valid'; user_subscription($in); }
}
if ($discount) $a[price] = $a[price] - ($a[price] * ($discount/100));
if ($a[price]<=0)
{ $user_vars = get_user_variables($s[LUG_u_n]);
  if ($user_vars[paid_until]<$s[cas]) $user_vars[paid_until] = $s[cas];
  $new_paid_until = strtotime("+".round($a[months])." months",$user_vars[paid_until]);
  
  $q = dq("select * from $s[pr]links_extra_orders where payment_type = 'user' and user = '$s[LUG_u_n]' and info = 'Coupon' and notes = '$_POST[discount]'",1);
  if (mysql_num_rows($q)<=0)
  { dq("insert into $s[pr]links_extra_orders values (NULL,'$s[LUG_u_n]','$s[cas]','$a[price]','1','','Coupon','$in[discount]','user','$s[LUG_u_n]','$a[months]')",1);
    $order_n = mysql_insert_id();
    dq("insert into $s[pr]users_payments values(NULL,'$order_n','$s[LUG_u_n]','$s[cas]','$a[months]','0','$a[price]','1','Coupon','0','0','0','',0)",1);
    $users_payments_n = mysql_insert_id();
    dq("update $s[pr]users_payments set paid_until = $new_paid_until where n  = '$users_payments_n'",1);
    dq("update $s[pr]users set paid_until = $new_paid_until where n = '$s[LUG_u_n]'",1);
  }
	
}

dq("insert into $s[pr]links_extra_orders values (NULL,'$s[LUG_u_n]','$s[cas]','$a[price]','0','','','','user','$s[LUG_u_n]','$months')",1);
$order_n = mysql_insert_id();
setcookie(LUG_payment,false);
setcookie(LUG_payment,$order_n,$s[cas]+600); 

dq("delete from $s[pr]payment_process where time < ($s[cas]-600) or user = '$s[LUG_u_n]'",1);
if ($_COOKIE[LUG_u_n]) $remember_me = 1;
dq("insert into $s[pr]payment_process values ('$s[ip]','$order_n','$s[LUG_u_n]','$s[cas]','$remember_me')",1);
...
$data[number] = $order_n; $data[price] = $a[price]; $data[months] = $a[months];
$a[paypal_form] = parse_part('order_form_paypal_users.txt',$data);

page_from_template('user_order.html',$a);
}
...
{ if (!$in[order_options]) $checked = ' checked'; else $checked = '';
  $in[order_options] .= '<input type="radio" name="months" value="'.$x[months].'"'.$checked.'>'.$x[months].' months for '.$s[currency].$x[price].'<br>';
}
...
if ((!$user_vars[n]) OR (md5($user_vars[password])!=$in[password])) problem($m[no_account]);
$code = md5($user_vars[joined]); if ($code!=$in[code]) problem($m[w_confirm]);
dq("update $s[pr]users set ip = '$s[ip]', confirmed = '1' where n = '$user_vars[n]'",1);
$in[username] = $in[user];
if ($s[i_admin_user_joined])
{ $user_vars[action] = 'joined';
  for ($x=1;$x<=3;$x++) $address[] = $user_vars["address$x"]; $address[] = $user_vars[country]; $user_vars[address] = implode(", ",$address);
  for ($x=1;$x<=3;$x++) $phone[] = $user_vars["phone$x"]; $user_vars[phones] = implode(", ",$phone);
  mail_from_template('user_joined_edited_admin.txt',$user_vars);
}
  create_other_gc1_script_user($user_vars[n]);
  create_other_gc2_script_user($user_vars[n]);
  ...
  $s[info] = info_line("Your account has been created.");
  user_subscription($in);
...
if ($user[showemail]) $user[showemail_checked] = ' checked';
$user[info] = $s[info];
...
{ if (!$s[user_no_auto]) $approved = 1;
  ...
  create_other_gc1_script_user($n);
  create_other_gc2_script_user($n);
}
}
...
{ $user_vars = get_user_variables($s[new_user_n]);
  $_SESSION[LUG_u_username] = $user_vars[username];
  $_SESSION[LUG_u_password] = $user_vars[password];
  $_SESSION[LUG_u_n] = $user_vars[n];
  $_SESSION[LUG_u_email] = $user_vars[email];
...
$months = round($_POST[months]);
$q = dq("select * from $s[pr]users_prices where months = '$months'",1);
$a=mysql_fetch_assoc($q);
if (!$a[months])
{ $q = dq("select * from $s[pr]users_prices order by months limit 1",1);
  $a=mysql_fetch_assoc($q);
}


if (($a[coupon]) AND ($a[coupon]==trim($_POST[discount]))) $discount = 100;
elseif (trim($_POST[discount]))
{ for ($x=1;$x<=10;$x++) if (trim($_POST[discount])==$s["discount_code$x"]) { $discount = $s["discount$x"]; break; }
  //if (!$discount) { $s[info] = 'Discount code is not valid'; user_subscription($_POST); }
}
if ($discount) $a[price] = $a[price] - ($a[price] * ($discount/100));
...
{ $user_vars = get_user_variables($s[LUG_u_n]);
  ...
  dq("insert into $s[pr]links_extra_orders values (NULL,'$s[LUG_u_n]','$s[cas]','$a[price]','1','','Coupon','$_POST[discount]','user','$s[LUG_u_n]','$a[months]')",1);
  $order_n = mysql_insert_id();
  dq("insert into $s[pr]users_payments values(NULL,'$order_n','$s[LUG_u_n]','$s[cas]','$a[months]','0','$a[price]','1','Coupon','0','0','0','',0)",1);
  $users_payments_n = mysql_insert_id();
  dq("update $s[pr]users_payments set paid_until = $new_paid_until where n  = '$users_payments_n'",1);
  ...
  $form[hide_payment_begin] = '<!--'; $form[hide_payment_end] = '-->';
}

else
{
dq("insert into $s[pr]links_extra_orders values (NULL,'$s[new_user_n]','$s[cas]','$a[price]','0','','','','user','$s[new_user_n]','$months')",1);
$order_n = mysql_insert_id();
setcookie(LUG_payment,false);
setcookie(LUG_payment,$order_n,$s[cas]+600); 

dq("delete from $s[pr]payment_process where time < ($s[cas]-600) or user = '$s[new_user_n]'",1);
if ($_COOKIE[LUG_u_n]) $remember_me = 1;
dq("insert into $s[pr]payment_process values ('$s[ip]','$order_n','$s[new_user_n]','$s[cas]','$remember_me')",1);
if ($s[pp_test]) $data[paypal_url] = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
else $data[paypal_url] = 'https://www.paypal.com/cgi-bin/webscr';
$data[pp_currency] = $s[pp_currency]; $data[pp_email] = $s[pp_email];
$data[number] = $order_n; $data[price] = $a[price]; $data[months] = $a[months];
$form[paypal_form] = parse_part('order_form_paypal_users.txt',$data);
  $form[hide_login_begin] = '<!--'; $form[hide_login_end] = '-->';

}

  
  //user_subscription($in);
  page_from_template('user_joined.html',$form);
}
}
...
if (!$in[username]) user_login_page();
elseif (!$in[password]) user_login_page();
if ($s[user_login_captcha]) { $x = check_entered_captcha($in[image_control]); if ($x) { $s[info] = $x; user_login_page($in); } }
