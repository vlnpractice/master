...
/*
// uncomment to connect your adsense account to the web search in link up gold
if ($_GET[q])
...
{ // create an adsense search form in google users area and find your partner id in this line:
  // <input type="hidden" name="cx" value="partner-pub-8996471553250935:e5zqh7-ew5s" />
  // copy this id to the URL below
...
template search_result_google.html:
...
//if ($_GET[bigboss]=='google'){ header("Location: $s[site_url]/search.php?cref=http%3A%2F%2Fwww.xxxxxxx.com%2Fkkk.xml%3Fx%3D$s[cas]&cof=FORID%3A9&q=$_GET[phrase]"); exit; }
//if ($_GET[q])
if ($_GET[bigboss]=='google')
{ $_GET[selected_google] = ' selected';
  $_GET[original_phrase] = $_GET[phrase];
...
//udelat kdyz neni action a search_kind tak shared_simple a and
...
{ if ($_GET[bigboss]=='google')
  ...
  { if ((!$_GET[action]) OR ($_GET[action]=='shared_simple'))
    { if (!trim($_GET[phrase])) { $url = "$s[site_url]/"; header ("Location: $url"); exit; }
	  else $url = "$s[site_url]/search/".rawurlencode(html_entity_decode($_GET[phrase]))."/";
...
//$_GET[phrase] = htmlentities($_GET[phrase],ENT_IGNORE,'UTF-8');
...
if ($what=='v') page_from_template('search_result_videos.html',$a);
...
{ $v = str_replace('(','',str_replace(')','',str_replace('[','',str_replace(']','',$v))));
  ...
  $array_where1[] = "($s[other_gc1_pr]ads.title $regexp OR $s[other_gc1_pr]ads.description $regexp OR $s[other_gc1_pr]ads.detail $regexp)";
  $array_where2[] = "($s[other_gc2_pr]ads.title $regexp OR $s[other_gc2_pr]ads.description $regexp OR $s[other_gc2_pr]ads.detail $regexp)";
}
...
$where = '('.implode(" $boolean ",$array_where1).")";
$q = dq("select * from $s[other_gc1_pr]ads where $where order by created desc limit 5",1);
while ($x=mysql_fetch_assoc($q)) { $ads[] = $x; $numbers[] = $x[n]; }
$a[gc1_items] = get_complete_classifieds($ads,$numbers);
if (!$a[gc1_items]) { $a[hide_gc1_begin] = '<!--'; $a[hide_gc1_end] = '<!--'; }
disconnect_other_gc1_db();

unset($ads,$numbers);
connect_other_gc2_db();
$where = '('.implode(" $boolean ",$array_where2).")";
$q = dq("select * from $s[other_gc2_pr]ads where $where order by created desc limit 5",1);
while ($x=mysql_fetch_assoc($q)) { $ads[] = $x; $numbers[] = $x[n]; }
$a[gc2_items] = get_complete_classifieds($ads,$numbers);
if (!$a[gc2_items]) { $a[hide_gc2_begin] = '<!--'; $a[hide_gc2_end] = '<!--'; }
disconnect_other_gc2_db();

  
$a[original_phrase] = trim($phrase);
if (!$have_results) { $s[info] = info_line($m[no_result]); search_form($a); }
$a[phrase] = rawurlencode(html_entity_decode($phrase)); $a[search_kind] = $search_kind;
$a[adlinks] = get_adlinks(0,$a[original_phrase]);
$a[meta_title] = $a[original_phrase];
$a[meta_description] = "$a[original_phrase], $s[site_name]";
...
function get_complete_classifieds($ads,$numbers) {
global $s;
...
{ $a[title_no_tag] = strip_tags($a[title]);
  ...
  $a[item_details_url] = "$s[other_gc2_url]/classified.php?n=$a[n]";
  $a[created] = datum($a[created],0);
  $complete_array[] = '<td valign="top" width="'.$width.'%" style="padding:0px;">'.parse_part('classified_a.txt',$a).'</td>';
  $pocet++;
}
...
{ $font_size = round(35 - ($pocet/2));
  ...
  if ($x1) { $pocet++; if ($pocet<=50) $words_array[] = '<a style="font-size:'.$font_size.'px"; href="'.$s[site_url].'/search.php?phrase='.rawurlencode(html_entity_decode($x1)).'">'.htmlentities($x1).'</a>'; }
}
shuffle($words_array);
$a = implode("\n",$words_array);
