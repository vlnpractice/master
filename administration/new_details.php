...
{ $q = dq("select * from $s[pr]news where n = '$n'",1);
  ...
  if (!$news[n]) problem ("Print Outs item #$n does not exist.");
...
case 'new_create'	: $action = 'new_created'; $info = 'Create a New Print Outs item'; break;
case 'new_edit'		: $action = 'new_edited'; $info = 'Edit Selected Print Outs item'; break;
case 'new_edited'	: $action = 'new_edited'; $info = 'Edit Selected Print Outs item'; break;
case 'new_copy'		: $action = 'new_created'; $info = 'Copy Selected Print Outs item'; $news[old_number] = $news[n]; $news[n] = 0; break;
}
...
ih();
echo info_line('Print Outs item Created');
show_one_new($n);
ift();
}
...
$s[info] = info_line('Print Outs item Updated');
...
ih();
echo info_line('Selected Print Outs item Has Been '.$Status);
...
recount_items_cats('n',$news[c]);
ih(); 
echo info_line('Selected Print Outs item Has Been Deleted');
echo '<br /><br /><a href="javascript: history.go(-1)">Back</a>';
ift();
}
