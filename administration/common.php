...
{ if ($in[n]) $images = get_item_files_pictures($what,$in[n],$queue);
  for ($x=1;$x<=$max;$x++)
  { echo '<tr>
    <td nowrap align="left" valign="top">'; if ($what=='n') echo 'Upload an image or a pdf file'; else echo 'Upload an image'; if ($max>1) echo ' #'.$x; echo '</td>
    ...
    { if (strtolower(substr($images[image_url][$in[n]][$x],-3))=='pdf') echo '<span class="text10"><br />Current file: <a class="link10" target="_blank" href="'.$images[image_url][$in[n]][$x].'">'.$images[image_url][$in[n]][$x].'</a><br>';
	  else echo '<span class="text10"><br />Current image '.image_preview_code($images[image_n][$in[n]][$x],$images[image_url][$in[n]][$x],preg_replace("/\/$in[n]-/","/$in[n]-big-",$images[image_url][$in[n]][$x]));
...
{ if (strtolower(substr($url,-3))=='pdf') {
	echo '<tr>
  <td nowrap align="left" valign="top">File </td>
  <td nowrap align="left" valign="top"><a class="link10" target="_blank" href="'.$url.'">'.$url.'</a><br>';
  echo '</td>
  </tr>';
}
  else {
