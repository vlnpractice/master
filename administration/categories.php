...
if ($in[n]) $category_url = category_url($data[use_for],$data[n],$data[alias_of],$data[name],1,$data[pagename],$data[rewrite_url],$sort,$direction);
echo '
...
<td align="left" valign="top">Rank</td>
  <td align="left" valign="top"><input class="field10" style="width:200px;" name="cat_group" maxlength=255 value="'.$data[cat_group].'"></td>
</tr>';
if (!$in[n])
{ echo '<tr>
...
{ if (!$in[n]) echo '<tr>
  ...
  echo '<tr>
  <td align="left" valign="top" nowrap>Only paying users can see this category </td>
  <td align="left" valign="top"><input type="checkbox" name="users_only_cat" value="1"'; if ($data[users_only_cat]) echo ' checked'; echo '></td>
  </tr>
  <tr>
  <td align="left" valign="top" nowrap>Only paying users can see details of '.$word.' from this category </td>
...
dq("delete from $s[pr]cats where alias_of = '$in[n]'",1);
