...
case 'do_daily_job'				: A_daily_job(0); break;
...
case 'A_rebuild_index'			: A_read_common_files(); A_rebuild_index(0); break;
case 'A_rebuild_categories_l'	: A_read_common_files(); A_rebuild_categories('l',0); break;
case 'A_rebuild_categories_a'	: A_read_common_files(); A_rebuild_categories('a',0); break;
case 'A_rebuild_categories_b'	: A_read_common_files(); A_rebuild_categories('b',0); break;
case 'A_rebuild_categories_v'	: A_read_common_files(); A_rebuild_categories('v',0); break;
case 'A_rebuild_categories_n'	: A_read_common_files(); A_rebuild_categories('n',0); break;
case 'A_rebuild_detail_l'		: A_read_common_files(); A_rebuild_detail_pages('l',0); break;
case 'A_rebuild_detail_a'		: A_read_common_files(); A_rebuild_detail_pages('a',0); break;
case 'A_rebuild_detail_b'		: A_read_common_files(); A_rebuild_detail_pages('b',0); break;
case 'A_rebuild_detail_v'		: A_read_common_files(); A_rebuild_detail_pages('v',0); break;
case 'A_rebuild_detail_n'		: A_read_common_files(); A_rebuild_detail_pages('n',0); break;
...
case 'do_delete_static_files'	: $s[info] = A_delete_static_files($_GET[directory]); break;
case 'rebuild_one_link'			: A_read_common_files(); A_rebuild_detail_l(0,$_GET[n]); break;
}

switch ($_POST[action]) {
//case 'reset_rebuild'			: A_reset_rebuild($_POST[what]);
case 'reset_all'				: reset_all(1);
}
A_reset_rebuild_home();

##################################################################################

function A_reset_rebuild_home() {
global $s;
if ($s[time_1]) increase_print_time(0,'end');
load_times();
if ($s[info]) echo info_line($s[info]);

?>

<form method="get" action="html_rebuild.php" name="form1">
...
<td align="left">Daily job (this job last done: <?PHP echo datum($s[times_d],1); ?>)<br /><span class="text10">It recounts statistics, rebuilds the home page and categories, deletes unconfirmed links older than 24 hours, deletes unconfirmed user accounts older than 24 hours, deletes IP records, updates popular links.<br />(This should be done automatically once a day, however if you need to do it manually, use this function)</span></td>
</tr>
<tr>
...
<tr><td class="common_table_top_cell" colspan=2>Rebuild Selected Pages</td></tr>
<tr>
<td align="left" nowrap><input type="radio" name="action" value="A_rebuild_index"></td>
<td align="left" nowrap>Home page <?PHP echo $s[Aindexhtml] ?> <span class="text10">(Part on the daily job)</span></td>
</tr>
<?PHP
foreach ($s[items_types_Words] as $k=>$v)
{ echo '<tr>
  <td align="left" valign="top" nowrap><input type="radio" name="action" value="A_rebuild_categories_'.$k.'"></td>
  <td align="left" valign="top" nowrap>'.$v.' - categories <span class="text10">(Part on the daily job)</span></td>
  </tr>
  <tr>
  <td align="left" valign="top" nowrap><input type="radio" name="action" value="A_rebuild_detail_'.$k.'"></td>
  <td align="left" valign="top" nowrap>'.$v.' - detail pages <span class="text10">(Part on the daily job)</span></td>
  </tr>';
...
<tr><td class="common_table_top_cell" colspan=2>Delete Static Pages</td></tr>
<tr>
<td align="left" valign="top" nowrap><input type="radio" name="action" value="do_delete_static_files"></td>
<td align="left" valign="top" nowrap>Delete all *.<?PHP echo $s[Ahtml_ex] ?> files in directory:</td>
</tr>
<tr>
<td align="left" valign="top" nowrap><input type="radio" name="directory" value="main"></td>
<td align="left" valign="top" nowrap>Main</td>
</tr>
<?PHP
foreach ($s[items_types_Words] as $k=>$v)
{ echo '<tr>
  <td align="left" nowrap><input type="radio" name="directory" value="'.$s['Afolder_'.$k.'_cat'].'"></td>
  <td align="left" nowrap>'.$s['Afolder_'.$k.'_cat'].'</td>
  </tr>
  <tr>
  <td align="left" nowrap><input type="radio" name="directory" value="'.$s['Afolder_'.$k.'_detail'].'"></td>
  <td align="left" nowrap>'.$s['Afolder_'.$k.'_detail'].'</td>
  </tr>';
}
?>
...
<td align="left" nowrap>Delete expired links, articles, blogs, videos and news <span class="text10"><br />Removes all items which are no more valid (have been valid in the past)</span></td>
</tr>
<tr>
