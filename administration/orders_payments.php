...
case 'orders_search_users'		: orders_search_users();
case 'orders_searched'			: orders_searched($_GET);
case 'orders_searched_users'	: orders_searched_users($_GET);
...
function orders_search_users() {
global $s;
ih();
echo page_title('Orders');
$q = dq("select count(*) from $s[pr]links_extra_orders where paid = '0' and payment_type = 'user'",1);
$pocet = mysql_fetch_row($q); $pocet = $pocet[0];
if ($pocet)
{ echo '  <form method="get" action="orders_payments.php">
  <input type="hidden" name="action" value="orders_searched_users">
...
<input type="hidden" name="action" value="orders_searched_users">
<table border="0" width="900" cellspacing="0" cellpadding="0" class="common_table">
<tr><td class="common_table_top_cell" colspan="2">Search for Membership Orders</td></tr>
<tr><td align="center" width="100%">
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="inside_table">
<tr>
...
<td align="left"><select class="select10" name="sort"><option value="order_time">Date created</option><option value="user">Usernumber</option></select><select class="select10" name="order"><option value="asc">Ascending</option><option value="desc">Descending</option></select></td>
</tr>
...
function orders_searched_users($in) {
global $s;
...
{ $rozcesti = '<form action="orders_payments.php" method="GET" name="form1"><input type="hidden" name="action" value="orders_searched_users">';
...
echo "$rozcesti<br />";
$in[from] = $in[from] + 1;
while ($order=mysql_fetch_assoc($q)) show_one_order_users($order);
ift();
}
############################################################################

function show_one_order_users($data) {
global $s;
...
<tr>
<td align="left" valign="top" nowrap>Payments </TD>
<td align="left">
<table border="0" width="100%" cellspacing="0" cellpadding="2">
<tr>
<td align="left" nowrap>#</TD>
<td align="left" nowrap>Date</TD>
<td align="left" nowrap>Paid until</TD>
<td align="left" nowrap>Successful</TD>
<td align="left" nowrap>Canceled</TD>
<td align="left" nowrap>Details</TD>
<td align="left" nowrap>Paypal ID\'s</TD>
</TR>';
$q = dq("select * from $s[pr]users_payments where order_n = '$data[n]' order by time desc",1);
while ($x = mysql_fetch_assoc($q))
{ if ($x[success]) $success = 'Yes'; else $success = 'No';
  if ($x[canceled]) $canceled = 'Yes'; else $canceled = 'No';
  echo '<tr>
  <td align="left" valign="top" nowrap>'.$x[n].'</TD>
  <td align="left" valign="top" nowrap>'.datum($x[time],1).'</TD>
  <td align="left" valign="top" nowrap>'.datum($x[paid_until],1).'</TD>
  <td align="left" valign="top" nowrap>'.$success.'</TD>
  <td align="left" valign="top" nowrap>'.$canceled.'</TD>
  <td align="left" valign="top">'.$x[info].'</TD>
  <td align="left" valign="top" nowrap>'."$x[paypal_subscription_id]<br>$x[paypal_transaction_id]<br>$x[paypal_ipn_id]<br>".'</TD>
  </TR>';
}
echo '</TD>
</table>
...
elseif ($order[payment_type]=='user') $message = "Months added to user account: ".round($order[days_clicks_or_value]);
ih();
