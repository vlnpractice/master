...
function email_users_renew($showresult) {
global $s;

$cas1 = $s[cas] + (29*86400); $cas2 = $s[cas] + (30*86400);
$q = dq("select * from $s[pr]users_payments where canceled = 0 and paid_until >= $cas1 and paid_until <= $cas2",1);
while ($x = mysql_fetch_assoc($q))
{ $user_vars = get_user_variables($x[user]); if (!$user[email]) continue;
  $x[to] = $user[email];
  $x[renew_date] = datum($x[paid_until]);
  mail_from_template('user_subscription_30.txt',$x);
}

$cas1 = $s[cas] + (13*86400); $cas2 = $s[cas] + (14*86400);
$q = dq("select * from $s[pr]users_payments where canceled = 0 and paid_until >= $cas1 and paid_until <= $cas2",1);
while ($x = mysql_fetch_assoc($q))
{ $user_vars = get_user_variables($x[user]); if (!$user[email]) continue;
  $x[to] = $user[email];
  $x[renew_date] = datum($x[paid_until]);
  mail_from_template('user_subscription_14.txt',$x);
}

$cas1 = $s[cas]; $cas2 = $s[cas] + (1*86400);
$q = dq("select * from $s[pr]users_payments where canceled = 0 and paid_until >= $cas1 and paid_until <= $cas2",1);
while ($x = mysql_fetch_assoc($q))
{ $user_vars = get_user_variables($x[user]); if (!$user[email]) continue;
  $x[to] = $user[email];
  $x[renew_date] = datum($x[paid_until]);
  mail_from_template('user_subscription_1.txt',$x);
}
...
{ $item = get_item_variables($x[what],$x[n]);
  ...
  $q1 = dq("select $s[pr]users.*,$s[pr]u_favorites.n as category from $s[pr]u_favorites,$s[pr]users where $s[pr]users.showemail = 1 and $s[pr]u_favorites.what = 'c_$x[what]' and ($query) and $s[pr]u_favorites.user = $s[pr]users.n",1);
...
{ $table = $s[item_types_tables][$what];
  ...
  $q = dq("select * from $s[pr]cats where use_for = '$what' AND visible = '1' AND level = '1' order by cat_group,name",1);
  ...
  /*
  $q = dq("select * from $table WHERE $s[where_fixed_part] order by created desc limit $s[right_column_items]",1);
  while ($x = mysql_fetch_assoc($q)) { $item1[] = $x; $numbers1[] = $x[n]; }
  if ($numbers1)
  { $function_name = 'get_complete_'.$s[items_types_words][$what];
    $static[home][Sky][$what][new_items1] = $function_name($item1,$numbers1,$s[item_types_words][$what].'_a.txt');
  }
  unset($item1,$numbers1);
  */
...
$menu = php_rebuild_parse_part('menu.txt','_common',$i);
$fp = fopen ("$s[phppath]/data/menu.txt",'w');
fwrite ($fp,$menu); fclose($fp); chmod("$s[phppath]/data/menu.txt",0666);
      ...
      { if (strstr($what,'_cats')) $out[$what] .= A_parse_part('_in_one_category.txt',$data[$what][$k]);
	    ...
	    { if ($mark=='first_cats') $out[$st][$what.'_first_cats'] .= php_rebuild_parse_part('_in_one_category.txt',$st,$item_array);
          ...
          { //foreach ($item_array as $k5=>$v5) echo "$k5 - $v5<br>";
            ...
            if ($what=='n') $new_items[$what][$st][] = php_rebuild_parse_part('index_new_item.txt',$st,$item_array);
            else $static[home][$st][$what][new_items] .= php_rebuild_parse_part('index_new_item.txt',$st,$item_array);
          }
        }
      }
    }
...
{ foreach ($s[styles_list] as $stlk=>$st)
  if ($what=='n') $static[home][$st][$what][new_items] = multicolumns_table($new_items[$what][$st],2,1,'php_rebuild_parse_part','index_new_item.txt',$st);
  else $static[home][$st][$what][new_items] = multicolumns_table($new_items[$what][$st],50,1,'php_rebuild_parse_part','index_new_item.txt',$st);
}
...
{ unset($menu_categories);
  $q = dq("select * from $s[pr]cats where in_menu = '1' and use_for = '$what' order by cat_group,name",1);
...
global $s;
if ($query = get_new_items($what,25))
{ unset($x,$numbers,$items);
  $table = $s[item_types_tables][$what];
  $q = dq("select * from $table where $query",1);
  while ($x = mysql_fetch_assoc($q))
  {	//$x[item_details_url] = get_detail_page_url($what,$x[n],$x[rewrite_url],0,1);
  //echo "$x[item_details_url]<br>";
...
//    else $a[$cislo][url] = get_detail_page_url($what,$x[n],$x[rewrite_url],0,1);
    ...
    if ($x[description]) $a[$cislo][description] = $x[description]; elseif ($what=='n') $a[$cislo][description] = strip_tags($x[text],'<img>'); else $a[$cislo][description] = my_substr(strip_tags($x[text],'<img>'),255);
        $a[$cislo][image_1] = $x[picture];
    if ($x[youtube_thumbnail]) $a[$cislo][picture] = '<a href="'.$a[$cislo][url].'"><img border="0" src="'.$x[youtube_thumbnail].'" alt="'.$x[title].'" width="130" height="80"></a>';

    else
    { $q = dq("select filename from $s[pr]files where item_n = '$x[n]' and what = '$what' and file_type = 'image' and not (filename like '%\.pdf') and queue = '0' order by file_n limit 1",1);
      $y = mysql_fetch_assoc($q);
      if (trim($y[filename])) $a[$cislo][picture] = '<a href="'.$a[$cislo][url].'"><img border="0" src="'.$y[filename].'" alt="'.$x[title].'" width="130" height="80"></a>';
      elseif (($what=='l') AND ($s[l_thumbnail_url])) { $x1 = parse_url($x[url]); $a[$cislo][picture] = '<a href="'.$a[$cislo][url].'"><img border="0" src="'.str_replace('#%domain%#',$x1[host],$s[l_thumbnail_url]).'" alt="'.$x[title].'" width="130" height="80"></a>'; }
    }
...
{ unset($a,$pocet,$categories);
  $q = dq("select * from $s[pr]cats where use_for = '$what' AND level = '1' AND visible = '1' AND hide_home = '0' order by cat_group,name",1);
  while ($c = mysql_fetch_assoc($q))
  { $a[total] = $a[total] + $c[items]; unset($subcats);
    ...
    if ($s[max_subc])
    { $r = dq("select * from $s[pr]cats where use_for = '$what' AND level = '2' AND visible = '1' AND hide_home = '0' AND parent = '$c[n]' order by cat_group,name limit $s[max_subc]",1);
...
{ if (!$s["section_$what"]) continue;
  ...
  $q = dq("select n,name from $s[pr]cats where use_for = '$what' AND level = 1 and visible = '1' and alias_of = '0' order by cat_group,name",0);
...
{ set_time_limit(300);
  ...
  if ($show_result) echo 'Print Outs category '.$c[name].' updated<br />';
}
if ($show_result) echo '</span>'.info_line('Print Outs have been recounted');
}
...
$s[no_twitter] = 1;
if (!$_GET[create_index_cat]) { $item_types = $s[item_types_short]; dq("delete from $s[pr]index_suggest",1); }
elseif (is_numeric($_GET[create_index_cat])) { $category_vars = get_category_variables($_GET[create_index_cat]); $item_types[0] = $category_vars[use_for]; }
else $item_types[0] = $_GET[create_index_cat];

foreach ($item_types as $k=>$what)
{ if (!$_GET[create_index_cat]) dq("delete from $s[pr]index where what = '$what'",1);
  $table = $s[item_types_tables][$what];
  if ($category_vars[n])
  { $where_fixed_part = get_where_fixed_part($what,0,$category_vars[n],$s[cas]);
    $q = dq("select $table.n from $table,$s[pr]cats_items where $where_fixed_part",1);
  }
  else
  { $where_fixed_part = $s[where_fixed_part];
    $q = dq("select n from $table where $where_fixed_part",1);
  }
