...
{ $q = dq("select * from $s[pr]cats_dmoz where n = '$parent'",1);
  $parent_vars = mysql_fetch_assoc($q);
  ...
  $big_parent_vars = mysql_fetch_assoc($q);
...
{ $q1 = dq("select count(*) from $s[pr]cats_dmoz where parent_path = '$cat[path]'",1); $subcategories = mysql_fetch_row($q1);
...
$q = dq("select max(level) from $s[pr]cats_dmoz where parent_path like '$parent_vars[path]%'",1); $level = mysql_fetch_row($q);
$q = dq("select * from $s[pr]cats_dmoz where parent_path = '$parent_vars[path]' OR parent_path like '$parent_vars[path]/%' order by level",1);

if (($num_subcats=mysql_num_rows($q)) AND (!$in[confirmed]))
...
{ if (!$in[levels]) $in[levels] = 0;
  ...
  if ($num_subcats=mysql_num_rows($q1))
