...
global $s;
dq("delete from $s[pr]users_prices",1);

foreach ($in[months] as $k=>$months)
{ $price = $in[price][$k];
  $coupon = $in[coupon][$k];
  if ((!$months) OR (!$price)) continue;
  dq("insert into $s[pr]users_prices values ('$months','$price','$coupon')",1);
}
...
{ echo '<tr>
 <td align="center"><input class="field10" name="months[]" value="'.$price[months].'" size=10 maxlength=10></td>
 <td align="center">'.$s[currency].'<input class="field10" name="price[]" value="'.$price[price].'" size=10 maxlength=10></td>
 <td align="center"><input class="field10" name="coupon[]" value="'.$price[coupon].'" size=10></td>
 </tr>';
}
for ($x=1;$x<=10;$x++)
{ echo '<tr>
 <td align="center"><input class="field10" name="months[]" size=10 maxlength=10></td>
 <td align="center">'.$s[currency].'<input class="field10" name="price[]" size=10 maxlength=10></td>
 <td align="center"><input class="field10" name="coupon[]" size=10></td>
 </tr>';
}
...
[<a target="_self" href="orders_payments.php?action=orders_searched_users&user='.$data[n].'&sort=order_time&order=desc&showtext=on">Membership orders</a>]&nbsp;&nbsp;
...
<td align="left">'.$s[currency].$user[funds_now].'</td>
 ...
 echo '<tr>
  <td align="left">Email user when a new item in a bookmarked category has been created </td>
  <td align="left"><input type="checkbox" name="user['.$n.'][showemail]" value="1"'; if ($user[showemail]) echo ' checked'; echo '></td>
  </tr>';
echo '<tr>
...
update_items_for_user($in[n]);
  update_other_gc1_script_user($in[n]);
  update_other_gc2_script_user($in[n]);
}
